...
static void volume_change_simulation(void *arg)///////////////////////////////////////////////////////////
{
    ...
    button_init(VAL_PLUS_BUTTON_GPIO); 
    button_init(VAL_MINUS_BUTTON_GPIO);
    uint8_t volume =s_volume;
    int button_state = 0;
    int button_state2 = 0;
    for (;;) {

        button_state = gpio_get_level(VAL_PLUS_BUTTON_GPIO);
        button_state2 = gpio_get_level(VAL_MINUS_BUTTON_GPIO);
        if (button_state == 1 && s_volume <= 120) {
            
            //printf("Val Plus Button === 1\n");
            volume = (s_volume +7);// & 0x7f;
            volume_set_by_local_host(volume);
        }   
        if (button_state2 == 1 && volume >= 7) { 
            //printf("Val Minus Button === 1\n");
            volume = (s_volume -7);
            volume_set_by_local_host(volume);
        } 
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_CONNECTION_STATE_EVT: {
        ...
        if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_DISCONNECTED) {
            ...
            gpio_set_level(LED_OK, 0);gpio_set_level(LED_ERROR, 1);
        } else if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED){
            esp_bt_gap_set_scan_mode(ESP_BT_NON_CONNECTABLE, ESP_BT_NON_DISCOVERABLE);
            bt_i2s_task_start_up();
        } else if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTING) {
            bt_i2s_driver_install();
            led_init(LED_OK); gpio_set_level(LED_OK, 1);gpio_set_level(LED_ERROR, 0);
        }
        break;
    }
